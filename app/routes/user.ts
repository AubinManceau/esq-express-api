 ...
 *       400:
 *         description: Requête invalide (champs manquants ou mots de passe non identiques)
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *               rolesCategories:
 *                 type: array
 *                 description: Liste des rôles et catégories à assigner à l'utilisateur. Les rôles 1 et 2 nécessitent obligatoirement une catégorie.
 *                 items:
 *                   type: object
 *                   properties:
 *                     roleId:
 ...
 *                       example: "2025-08-27T12:00:00Z"
 *       400:
 *         description: Requête invalide (paramètres manquants ou contraintes non respectées)
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                 message:
 *                   type: string
 *                   example: La catégorie est requise pour le rôle 'Coach'
 *       404:
 ...
 *                   example: Erreur interne du serveur lors de la mise à jour de l'utilisateur.
 */
router.patch('/admin/:userId', auth, upload.fields([{ name: 'photo', maxCount: 1 }, { name: 'photo_celebration', maxCount: 1 }]), role([ROLES.ADMIN]), userCtrl.updateUserForAdmin);
/**
 * @swagger
 * /users/{userId}:
 *   delete:
 ...
 *                 message:
 *                   type: string
 *                   example: L'identifiant de l'utilisateur est requis.
 *       404:
 *         description: Utilisateur non trouvé
 *         content:
 *           application/json:
 ...
 */
router.delete('/:userId', express.json(), auth, role([ROLES.ADMIN]), userCtrl.deleteUser);
/**
 * @swagger
 * /users/{userId}:
 *   get:
...
router.get('/uploads/:filename', express.json(), auth, role([ROLES.MEMBER, ROLES.ADMIN]), userCtrl.getFiles);
