...
const createGroupChat = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (categoryId) {
            const categoryExists = await models.Categories.findByPk(categoryId as any, { transaction: t });
            if (!categoryExists) {
                await t.rollback();
        ...
        if (roleId) {
            const roleExists = await models.Roles.findByPk(roleId as any, { transaction: t });
            if (!roleExists) {
                await t.rollback();
                return res.status(404).json({
                    status: 'error',
        ...
        if (userIds.length > 0) {
            // @ts-ignore
            if ((groupChat as any).addUsers) {
                await (groupChat as any).addUsers(userIds, { transaction: t });
            }
        }
...
const updateGroupChat = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const groupChat = await models.ChatGroups.findByPk(id as any, { transaction: t });
        if (!groupChat) {
            await t.rollback();
            return res.status(404).json({
                status: 'error',
        ...
        // @ts-ignore
        if ((groupChat as any).setUsers) {
            await (groupChat as any).setUsers(userIds, { transaction: t });
        }
        await redis.del('groupChats:{}{}');
        await t.commit();
...
const getAllGroupChats = async (req: Request, res: Response) => {
    try {
...
const getGroupChatById = async (req: Request, res: Response) => {
    try {
        const userId = (req.auth as any).userId;
        const { id } = req.params;
        const groupChat = await models.ChatGroups.findOne({ where: { id: id } as any,
            include: [
                {
                    model: models.Users,
                    where: { id: userId } as any,
